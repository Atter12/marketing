...
export default function App({ role = "gerente", clientId = null, userEmail = null }) {
  ...
  const [cofPeriodoActual, setCofPeriodoActual] = useState(false);
  ...
  /* ═══ MODALS ═══ */
  const openMdl = (type, eid = null) => { setEditId(eid); setModal(type); if (type === "cobro" && !eid) { setCof(emptyCof); setCofFilterCliente(""); setCofFilterPeriodo(""); setCofPeriodoActual(false); } if (type === "garantia" && !eid) setGafFilterPeriodo(""); };
  const closeMdl = () => { setModal(null); setEditId(null); setCf(emptyCf); setGf({ ...emptyGf, clientId: curCl || "" }); setCof(emptyCof); setGaf({ ...emptyGaf, clientId: curCl || "" }); setAccesoResultado(null); setCobroComprobanteFiles([]); setCofGastosQuery(""); setCofFilterCliente(""); setCofFilterPeriodo(""); setCofPeriodoActual(false); setGafFilterPeriodo(""); setGarantiaImagenNewFiles([]); setGfClientNameInput(""); };
        ...
        <div className="hm-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
          ...
          <div>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, cursor: "pointer", fontSize: 13 }}>
              <input type="checkbox" checked={cofPeriodoActual} onChange={(e) => { const v = e.target.checked; setCofPeriodoActual(v); if (v) setCofFilterPeriodo(tm()); else setCofFilterPeriodo(""); }} style={{ width: 16, height: 16, accentColor: "#0055ff" }} />
              <span>Periodo actual</span>
            </label>
            <input type="text" placeholder="MM/AAAA" value={cofPeriodoActual ? tm() : cofFilterPeriodo} onChange={(e) => { if (!cofPeriodoActual) setCofFilterPeriodo(e.target.value); }} onBlur={(e) => { if (!cofPeriodoActual) { const p = parsePeriodoInput(e.target.value); if (p) setCofFilterPeriodo(p); } }} disabled={cofPeriodoActual} style={{ width: "100%", boxSizing: "border-box", padding: "9px 13px", background: cofPeriodoActual ? "#f0f4ff" : "#fff", border: "1px solid #e2e4e9", borderRadius: 8, fontFamily: "'DM Sans',sans-serif", fontSize: 13, outline: "none", opacity: cofPeriodoActual ? 0.9 : 1 }} />
          </div>
        </div>
        </>
        )}
