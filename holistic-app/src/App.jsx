...
export default function App({ role = "gerente", clientId = null, userEmail = null }) {
  ...
  const scrollRefReportes = useRef(null);
  const scrollRefClientes = useRef(null);
  const scrollRefGastos = useRef(null);
  const scrollRefCobros = useRef(null);
  const scrollRefGarantias = useRef(null);
  ...
  return (
    <div className={"hm-app" + (menuOpen ? " menu-open" : "")} style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans',-apple-system,BlinkMacSystemFont,sans-serif", background: "#f8fafc", color: "#0f172a", WebkitFontSmoothing: "antialiased", zoom: 0.9 }}>
...
.hm-table-wrap.hm-table-reportes table{min-width:900px}
...
.hm-scroll-hint{position:absolute;top:12px;bottom:auto;right:14px;width:32px;height:32px;padding:0;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.95);border:1px solid #e5e7eb;border-radius:10px;color:#64748b;font-size:18px;cursor:pointer;z-index:2;box-shadow:0 2px 8px rgba(15,23,42,.08);transition:background .15s,color .15s}
.hm-scroll-hint:hover{background:#f1f5f9;color:#0f172a}
.hm-scroll-hint-left{right:auto;left:14px}
...
`}</style>
        ...
        {page === "dashboard" && isCliente && (<div>
          ...
          <div className="hm-page-content" style={{ padding: "32px 48px 48px", maxWidth: 1440, margin: "0 auto" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, overflow: "hidden", boxShadow: "0 1px 4px rgba(15,23,42,.04)" }}>
              ...
              <div className="hm-table-wrap hm-table-dashboard"><table><thead><tr>{["Cliente", "Fecha de movimiento", "Gasto", "Fee", "Total", "Pagado", "Pendiente", "Estado"].map((h) => <th key={h} style={TH}>{h}</th>)}</tr></thead>
          ...
          <div className="hm-page-content" style={{ padding: "32px 48px 48px", maxWidth: 1440, margin: "0 auto" }}>
            ...
            <div className="hm-report-grid" style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 16 }}>
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, overflow: "hidden", boxShadow: "0 1px 4px rgba(15,23,42,.04)" }}>
                ...
                <div style={{ position: "relative", width: "100%" }}><div ref={scrollRefReportes} className="hm-table-wrap hm-table-reportes"><table><thead><tr>{["Usuario", "ADS", "FEE", "FEE %", "TOTAL", "PAGADO", "GARANTÍA", "PEND. NETO"].map((h) => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                  <tbody>
                  ...
                  </tbody></table></div><button type="button" className="hm-scroll-hint hm-scroll-hint-left" onClick={() => scrollRefReportes.current?.scrollBy({ left: -280, behavior: "smooth" })} title="Deslizá a la izquierda" aria-label="Scroll izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></button><button type="button" className="hm-scroll-hint" onClick={() => scrollRefReportes.current?.scrollBy({ left: 280, behavior: "smooth" })} title="Deslizá a la derecha para ver más columnas" aria-label="Scroll derecha"><ChevronRight size={20} strokeWidth={2.5} /></button></div>
              </div>
              <div className="hm-chart-card" style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, padding: "26px 28px", transition: "all .2s ease", boxShadow: "0 1px 4px rgba(15,23,42,.03)", display: "flex", flexDirection: "column", alignItems: "center" }}>
                <h4 style={{ fontSize: 15, fontWeight: 700, marginBottom: 4, letterSpacing: -0.2, alignSelf: "flex-start" }}>Composición de Cuentas</h4>
                <ResponsiveContainer width="100%" height={240}><PieChart><Pie data={repData.pie} cx="50%" cy="50%" innerRadius={62} outerRadius={98} paddingAngle={5} cornerRadius={4} dataKey="value" label={cLabel}>{repData.pie.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ borderRadius: 12, fontSize: 13, border: "none", boxShadow: "0 8px 30px rgba(15,23,42,.12)", padding: "10px 14px" }} formatter={(v) => `$${fmt(v)}`} /><Legend wrapperStyle={{ fontSize: 12 }} formatter={(v, e) => `${v}: $${fmt(e.payload.value)}`} /></PieChart></ResponsiveContainer>
              ...
              return (
                <>
                  ...
                  <div style={{ position: "relative", width: "100%" }}><div ref={scrollRefCobros} className="hm-table-wrap hm-table-sticky-actions hm-table-cobros"><table><thead><tr><th style={{ ...TH, width: 42 }}><input type="checkbox" checked={allOnPageSelected} onChange={() => selectAllOnPage("cobros", idsOnPage)} title="Seleccionar página" style={{ cursor: "pointer", width: 16, height: 16 }} /></th>{["Cliente", "Cód. cobro", "Cód. gasto", "Fecha", "Hora", "Ref.", "Monto", "Método", "Registrado por", "Registrado", "Notas", "Comprob.", ""].map((h) => <th key={h || "actions"} style={TH} className={h === "" ? "hm-col-actions" : undefined}>{h}</th>)}</tr></thead>
                    ...
                    })}{!cobrosPaginated.length && <Empty cols={14} msg={emptyCobrosMsg} />}</tbody></table></div><button type="button" className="hm-scroll-hint hm-scroll-hint-left" onClick={() => scrollRefCobros.current?.scrollBy({ left: -280, behavior: "smooth" })} title="Deslizá a la izquierda" aria-label="Scroll izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></button><button type="button" className="hm-scroll-hint" onClick={() => scrollRefCobros.current?.scrollBy({ left: 280, behavior: "smooth" })} title="Deslizá a la derecha para ver más columnas" aria-label="Scroll derecha"><ChevronRight size={20} strokeWidth={2.5} /></button></div>
                  {totalPages > 1 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "12px 16px", borderTop: "1px solid #e5e7eb", flexWrap: "wrap" }}>
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <div style={{ position: "relative", width: "100%" }}><div ref={scrollRefGarantias} className="hm-table-wrap hm-table-sticky-actions hm-table-garantias"><table><thead><tr>{!isCliente && <th style={{ ...TH, width: 42 }}><input type="checkbox" checked={allOnPageSelected} onChange={() => selectAllOnPage("garantias", idsOnPage)} title="Seleccionar página" style={{ cursor: "pointer", width: 16, height: 16 }} /></th>}{["Cliente", "Cód. verificación", "Cód. gasto", "Gasto ($)", "Tipo", "Descripción", "Valor", "Estado", "Fecha colocación", "Registrado por", "Registrado", "Imágenes", ""].map((h) => <th key={h || "actions"} style={TH} className={h === "" ? "hm-col-actions" : undefined}>{h}</th>)}</tr></thead>
                    ...
                    })}{!garantiasPaginated.length && <Empty cols={colsCount + 1} msg={emptyGarantiasMsg} />}</tbody></table></div><button type="button" className="hm-scroll-hint hm-scroll-hint-left" onClick={() => scrollRefGarantias.current?.scrollBy({ left: -280, behavior: "smooth" })} title="Deslizá a la izquierda" aria-label="Scroll izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></button><button type="button" className="hm-scroll-hint" onClick={() => scrollRefGarantias.current?.scrollBy({ left: 280, behavior: "smooth" })} title="Deslizá a la derecha para ver más columnas" aria-label="Scroll derecha"><ChevronRight size={20} strokeWidth={2.5} /></button></div>
                  {totalPages > 1 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "12px 16px", borderTop: "1px solid #e5e7eb", flexWrap: "wrap" }}>
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
      ...
      <Mdl open={modal === "cobro"} onClose={closeMdl} title={editId ? "Editar Cobro" : "Registrar Cobro"} footer={<><Btn variant="outline" onClick={closeMdl} disabled={uploadingComprobantes}>Cancelar</Btn><Btn variant="accent" onClick={saveCobro} disabled={uploadingComprobantes}>{uploadingComprobantes ? "Subiendo…" : editId ? "Guardar" : "Registrar"}</Btn></>}>
        ...
        <div style={{ marginBottom: 16, padding: "18px 20px", background: "linear-gradient(135deg, #f8fafc, #eff6ff)", border: "1.5px solid #bfdbfe", borderRadius: 16, position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ fontSize: 13, color: "#334155", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>Total pagado: <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 700, color: "#059669", fontSize: 16, padding: "2px 8px", background: "#ecfdf5", borderRadius: 6 }}>${fmt(parseFloat(cof.monto) || 0)}</span></div>
          <div style={{ overflowX: "auto", maxHeight: 220, overflowY: "auto", borderRadius: 10, border: "1px solid #e5e7eb", background: "#fff" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
              ...
              <tbody>{resumenCobro.lineas.filter((l) => l.montoAplicado > 0).map((l, idx) => <tr key={l.gastoId} style={{ borderBottom: "1px solid #f1f5f9", background: idx % 2 === 0 ? "#fff" : "#fafbfc" }}><td style={{ padding: "10px 12px", fontWeight: 500 }}>{l.camp}</td><td style={{ padding: "10px 12px" }}>{fmtM(l.mes)}</td><td style={{ padding: "10px 12px", fontFamily: "'IBM Plex Mono',monospace", fontSize: 11.5, color: "#0f172a" }}>{l.codigo}</td><td style={{ padding: "10px 12px", textAlign: "right", fontFamily: "'IBM Plex Mono',monospace" }}>${fmt(l.pendienteAntes)}</td><td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 700, color: "#059669", fontFamily: "'IBM Plex Mono',monospace" }}>+${fmt(l.montoAplicado)}</td><td style={{ padding: "10px 12px", textAlign: "right", fontFamily: "'IBM Plex Mono',monospace", color: l.pendienteDespues > 0 ? "#e11d48" : "#059669", fontWeight: 600 }}>${fmt(l.pendienteDespues)}</td></tr>)}</tbody>
            </table>
          </div>
        ...
        <div style={{ marginTop: 8 }}>
          ...
          <label style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 18px", border: "2px dashed #3b82f6", borderRadius: 12, background: "linear-gradient(135deg, #eff6ff, #dbeafe)", color: "#1d4ed8", fontSize: 13, fontWeight: 600, cursor: uploadingComprobantes ? "wait" : "pointer", transition: "all .2s" }}>
            <Paperclip size={16} />
          ...
          if (accesoResultado) {
            return (
              <div style={{ padding: "16px 0" }}>
                ...
                <div style={{ background: "linear-gradient(135deg, #f8fafc, #ecfdf5)", borderRadius: 14, padding: 20, fontSize: 14, border: "1px solid #d1fae5" }}>
