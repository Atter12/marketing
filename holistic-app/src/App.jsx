...
export default function App({ role = "gerente", clientId = null, userEmail = null }) {
  ...
  const manualTabContent = detailTab === "manual" ? (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, overflow: "hidden", boxShadow: "0 1px 4px rgba(15,23,42,.04)" }}>
      <div style={{ position: "relative", width: "100%" }}><div className="hm-table-wrap hm-table-sticky-actions hm-table-manual"><table><thead><tr>{["Fecha", "Concepto", "Monto", "Tipo", "Nota", ...(isCliente ? [] : [""])].map((h) => <th key={h || "actions"} style={TH} className={h === "" ? "hm-col-actions" : undefined}>{h}</th>)}</tr></thead><tbody>{curManDisplay.map((m) => <tr key={m.id}><td style={TD}>{fmtD(m.fecha)}</td><td style={{ ...TD, fontWeight: 600 }}>{m.conc}</td><td style={{ ...TD, ...MN, color: m.tipo === "Gasto" ? "#dc2640" : m.tipo === "Ingreso" ? "#0d9f6e" : "#1a1d26" }}>{m.tipo === "Nota" ? "—" : (m.tipo === "Gasto" ? "-$" : "+$") + fmt(m.monto)}</td><td style={TD}><Bdg type={m.tipo === "Gasto" ? "err" : m.tipo === "Ingreso" ? "ok" : "n"}>{m.tipo}</Bdg></td><td style={{ ...TD, fontSize: 12.5, color: "#94a3b8" }}>{m.nota || "—"}</td>{!isCliente && <td className="hm-col-actions" style={TD}><IBtn onClick={() => mutations.delManual(m.id)} icon={<Trash2 size={13} />} danger /></td>}</tr>)}{!curManDisplay.length && <Empty cols={isCliente ? 5 : 6} msg="Sin registros" />}</tbody></table></div><span className="hm-scroll-hint hm-scroll-hint-left" aria-hidden="true" title="Deslizá a la izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></span><span className="hm-scroll-hint" aria-hidden="true" title="Deslizá a la derecha para ver más columnas"><ChevronRight size={20} strokeWidth={2.5} /></span></div>
...
.hm-scroll-hint{position:absolute;right:14px;bottom:14px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.95);border:1px solid #e5e7eb;border-radius:10px;color:#64748b;font-size:18px;pointer-events:none;z-index:2;box-shadow:0 2px 8px rgba(15,23,42,.08)}
        ...
        {page === "dashboard" && isCliente && (<div>
          ...
          <div className="hm-page-content" style={{ padding: "32px 48px 48px", maxWidth: 1440, margin: "0 auto" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, overflow: "hidden", boxShadow: "0 1px 4px rgba(15,23,42,.04)" }}>
              ...
              <div style={{ position: "relative", width: "100%" }}><div className="hm-table-wrap hm-table-dashboard"><table><thead><tr>{["Cliente", "Fecha de movimiento", "Gasto", "Fee", "Total", "Pagado", "Pendiente", "Estado"].map((h) => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                ...
                {dashboardPendientes.length === 0 && <Empty cols={8} msg={dashboardPeriodo ? "Sin pendientes en este período" : "🎉 Todo cobrado — sin pendientes"} />}</tbody></table></div><span className="hm-scroll-hint hm-scroll-hint-left" aria-hidden="true" title="Deslizá a la izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></span><span className="hm-scroll-hint" aria-hidden="true" title="Deslizá a la derecha para ver más columnas"><ChevronRight size={20} strokeWidth={2.5} /></span></div>
            </div>
          </div>
        </div>)}
          ...
          <div className="hm-page-content" style={{ padding: "32px 48px 48px", maxWidth: 1440, margin: "0 auto" }}>
            ...
            <div className="hm-report-grid" style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 16 }}>
              <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 18, overflow: "hidden", boxShadow: "0 1px 4px rgba(15,23,42,.04)" }}>
                ...
                <div style={{ position: "relative", width: "100%" }}><div className="hm-table-wrap hm-table-reportes"><table><thead><tr>{["Usuario", "ADS", "FEE", "FEE %", "TOTAL", "PAGADO", "GARANTÍA", "PEND. NETO"].map((h) => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                  <tbody>
                  ...
                  </tbody></table></div><span className="hm-scroll-hint hm-scroll-hint-left" aria-hidden="true" title="Deslizá a la izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></span><span className="hm-scroll-hint" aria-hidden="true" title="Deslizá a la derecha para ver más columnas"><ChevronRight size={20} strokeWidth={2.5} /></span></div>
              </div>
              ...
              return (
                <>
                  ...
                  <div style={{ position: "relative", width: "100%" }}><div className="hm-table-wrap hm-table-sticky-actions hm-table-cobros"><table><thead><tr><th style={{ ...TH, width: 42 }}><input type="checkbox" checked={allOnPageSelected} onChange={() => selectAllOnPage("cobros", idsOnPage)} title="Seleccionar página" style={{ cursor: "pointer", width: 16, height: 16 }} /></th>{["Cliente", "Cód. cobro", "Cód. gasto", "Fecha", "Hora", "Ref.", "Monto", "Método", "Registrado por", "Registrado", "Notas", "Comprob.", ""].map((h) => <th key={h || "actions"} style={TH} className={h === "" ? "hm-col-actions" : undefined}>{h}</th>)}</tr></thead>
                    ...
                    })}{!cobrosPaginated.length && <Empty cols={14} msg={emptyCobrosMsg} />}</tbody></table></div><span className="hm-scroll-hint hm-scroll-hint-left" aria-hidden="true" title="Deslizá a la izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></span><span className="hm-scroll-hint" aria-hidden="true" title="Deslizá a la derecha para ver más columnas"><ChevronRight size={20} strokeWidth={2.5} /></span></div>
                  {totalPages > 1 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "12px 16px", borderTop: "1px solid #e5e7eb", flexWrap: "wrap" }}>
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              ...
              return (
                <>
                  ...
                  <div style={{ position: "relative", width: "100%" }}><div className="hm-table-wrap hm-table-sticky-actions hm-table-garantias"><table><thead><tr>{!isCliente && <th style={{ ...TH, width: 42 }}><input type="checkbox" checked={allOnPageSelected} onChange={() => selectAllOnPage("garantias", idsOnPage)} title="Seleccionar página" style={{ cursor: "pointer", width: 16, height: 16 }} /></th>}{["Cliente", "Cód. verificación", "Cód. gasto", "Gasto ($)", "Tipo", "Descripción", "Valor", "Estado", "Fecha colocación", "Registrado por", "Registrado", "Imágenes", ""].map((h) => <th key={h || "actions"} style={TH} className={h === "" ? "hm-col-actions" : undefined}>{h}</th>)}</tr></thead>
                    ...
                    })}{!garantiasPaginated.length && <Empty cols={colsCount + 1} msg={emptyGarantiasMsg} />}</tbody></table></div><span className="hm-scroll-hint hm-scroll-hint-left" aria-hidden="true" title="Deslizá a la izquierda"><ChevronLeft size={20} strokeWidth={2.5} /></span><span className="hm-scroll-hint" aria-hidden="true" title="Deslizá a la derecha para ver más columnas"><ChevronRight size={20} strokeWidth={2.5} /></span></div>
                  {totalPages > 1 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "12px 16px", borderTop: "1px solid #e5e7eb", flexWrap: "wrap" }}>
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
