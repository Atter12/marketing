...
export default function App({ role = "gerente", clientId = null, userEmail = null }) {
  ...
  /** Consola: diagnóstico tipografía solo en dev (evita ruido en prod; manual: `window.__HM_LOG_FONTS__()`). */
  useEffect(() => {
  ...
  const [cobranzaJump, setCobranzaJump] = useState(null);
  const clearCobranzaJump = useCallback(() => setCobranzaJump(null), []);
  const jumpToCobranzaCliente = useCallback((c) => {
    setCobranzaJump({ key: Date.now(), search: (c.name || "").trim(), periodoYM: tm() });
    setPage("cobranza");
    setMenuOpen(false);
    if (typeof window !== "undefined") window.location.hash = "cobranza";
  }, []);
...
`}</style>
      ...
      <Mdl open={modal === "client"} onClose={closeMdl} title={editId ? "Editar Cliente" : "Nuevo Cliente"} footer={<><Btn variant="outline" onClick={closeMdl}>Cancelar</Btn><Btn onClick={saveClient}>Guardar</Btn></>}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <p style={{ fontSize: 11, color: "var(--sidebar-text-muted)", margin: "0 0 10px", lineHeight: 1.45 }}>Solo <strong>subir archivo</strong> (JPG, PNG, WebP o GIF; se guarda en Supabase, mismo criterio que Pendientes si el cliente está vinculado).</p>
